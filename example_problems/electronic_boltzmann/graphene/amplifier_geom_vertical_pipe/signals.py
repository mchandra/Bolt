...
#    file_number = -1
    ...
    emitter_indices   = q2 < 0.5+2*dy
    collector_indices = emitter_indices
    base_indices      = (q1 > 0.625-2*dx) & (q1 < 0.875+2*dx)
    ...
    base      = np.sum(vel_drift_y[-1, base_indices]*dx)
...
print("Amplification factor : collector/emitter = ", collector_array[time_index][-1]/emitter_array[time_index][-1])
