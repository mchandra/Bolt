...
sensor_1_left_start = -0.5 # um
sensor_1_left_end   = -0.25 # um

sensor_1_right_start = -0.5 # um
sensor_1_right_end   = -0.25 # um
...
skip_files = 5000 # Corresponds to t = 149.937 ps
moment_files = moment_files[skip_files:]


#time_array = params.dt_dump_moments * np.arange(0, moment_files.size, 1)
time_array = 149.937 + params.dt_dump_moments*np.arange(0, moment_files.size, 1)
...
time_indices = time_array > 150

#print (moment_files[skip_files])

pl.plot(time_array[time_indices], signal_array[time_indices])
pl.xlabel("Time (ps)")
