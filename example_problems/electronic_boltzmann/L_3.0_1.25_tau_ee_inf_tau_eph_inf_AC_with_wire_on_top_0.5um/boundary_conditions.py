...
def f_left(f, t, q1, q2, p1, p2, p3, params):
    ...
    vel_drift_x_in  = params.DC_offset + params.vel_drift_x_in*np.sin(omega*t)

    # TODO : Opening a port at the top left
    ...
    if (params.contact_geometry=="straight"):
        ...
        cond_in = ((params.y >= q2_contact_start) & \
                (params.y <= q2_contact_end) \
               )
        cond_out = ((params.y >= 0.75) & (params.y <= 1.5))
...
def f_right(f, t, q1, q2, p1, p2, p3, params):
    ...
    t     = params.current_time
    omega = 2. * np.pi * params.AC_freq
    #vel_drift_x_out = params.DC_offset + params.vel_drift_x_out*np.sin(omega*t)
    vel_drift_x_out = params.vel_drift_x*0.0
