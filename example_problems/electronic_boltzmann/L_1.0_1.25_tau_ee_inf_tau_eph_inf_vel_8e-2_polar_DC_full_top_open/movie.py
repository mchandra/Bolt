...
from matplotlib import transforms, colors
#pl.figure(figsize=(7.5*1.5, 1.5*25.0*10./6.5))
...
for file_number, dump_file in enumerate(moment_files[:]):
    ...
    print(moment_files[start_index+file_number])
    ...
    density_min = np.min(density[60:, :])
    density_max = np.max(density[60:, :])
...
#    plot_grid(x[::1, ::1], y[::1, ::1], alpha=0.5)
    pl.contourf(x, y, density.T, 100, norm=MidpointNormalize(midpoint=0, vmin=density_min, vmax=density_max), cmap='bwr', transform = rot + base)
    #pl.contourf(x[:, :], y[:, :],
    #    (density.T)[:, :], 200,
    #    norm=colors.SymLogNorm(linthresh=density.max()/20, vmin=density_min, vmax=density_max),
...
#    pl.colorbar()
    ...
    pl.streamplot(x[:, 0], y[0, :], 
                  j_x_m, j_y_m,
                  density=4, color='k',
                  linewidth=1, arrowsize=1.2, transform = rot + base
                 )
    pl.xlim([0, 5])
    pl.ylim([-1.25, 0])
    ...
    pl.savefig('images/dump_' + '%06d'%(file_number) + '.png', transparent = False)
    pl.clf()
