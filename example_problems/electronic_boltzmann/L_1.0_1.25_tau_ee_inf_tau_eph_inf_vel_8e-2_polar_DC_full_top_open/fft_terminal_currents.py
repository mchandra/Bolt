...
for file_number, dump_file in enumerate(moment_files):
    ...
    try :
        ...
        left_edge_currents.append(j_x[:, 0])
        right_edge_currents.append(j_x[:, -1])
        top_edge_currents.append(j_y[-1, :])
    
    except (IndexError):
        none_array = np.zeros(shape=(N_q2,))
        none_array.fill(np.nan)
        #print (none_array)
        left_edge_currents.append(none_array)
        right_edge_currents.append(none_array)
        top_edge_currents.append(none_array)
        print ("######################Index Error handled at file number ", file_number)


currents_left  = np.array(left_edge_currents)
currents_right = np.array(right_edge_currents)
currents_top   = np.array(top_edge_currents)

#print("Left : ", voltage_left.shape)

np.savez_compressed("diode_left_in_vel_0.2",
        left=currents_left, right=currents_right, top=currents_top)
