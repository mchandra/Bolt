...
#        if ((q2_midpoint > 0.33) and (q1_midpoint > -0.33) and (q1_midpoint < 0.33)):
#
#            # Determine the index of the vertical bar from the left edge of this patch
#            N = 4 # TODO :  Set the patches in params
#            index = 0 # Initialize index to zero
#            for i in range(N):
#                if (q1_midpoint < (-0.33 + i*0.66/N)):
#                    # Check the other limit
#                    if(q1_midpoint > (-0.33 + (i-1)*0.66/N)):
#                        index = i
#                        break
#
#            print ("Rank, Index = ", params.rank, index)
#                            
#            x_0 = -radius/np.sqrt(2)
#            x_n           = x_0 + np.sqrt(2)*radius*index/N # Left points
#            x_n_plus_1    = x_0 + np.sqrt(2)*radius*(index+1)/N # Right points
#            x_n_plus_half = x_0 + np.sqrt(2)*radius*(index+0.5)/N # Center points
#
#            x_y_bottom_left   = [x_n,           np.sqrt(radius**2 - x_n**2)]
#            x_y_bottom_center = [x_n_plus_half, np.sqrt(radius**2 - x_n_plus_half**2)]
#            x_y_bottom_right  = [x_n_plus_1,    np.sqrt(radius**2 - x_n_plus_1**2)]
#            
#            x_y_left_center   = [x_n,        (1+radius/np.sqrt(2))/2]
#            x_y_right_center  = [x_n_plus_1, (1+radius/np.sqrt(2))/2]
#            
#            x_y_top_left      = [x_n,           1]
