...
#    density = np.sqrt(vel_drift_x**2 + vel_drift_y**2)


    print (density[int(3.*N_q2/4.), 2])
#    pl.plot(y[0, :], vel_drift_x[:, 0])
#    pl.plot(y[0, :], vel_drift_x[:, -1])
#    pl.xlabel('y ($\mu$m)')
    #pl.plot(x[q1 < 0.4, 0], mu[int(N_q2/2), q1 < 0.4], '-o')
    #pl.plot(x[q1 < 0.4, 0][:-1], (mu[int(N_q2/2), q1 < 0.4] - np.roll(mu[int(N_q2/2), q1 < 0.4], -1, axis=0))[:-1], '-o')
    pl.xlabel('x ($\mu$m)')
    pl.plot(x[:, 0], density[int(3.*N_q2/4.), :], '-o')
    pl.ylim([-1e-10, 1e-10])
    pl.savefig('iv.png')
    pl.clf()
    ...
    pl.contourf(x, y, density.T, 1000, norm=MidpointNormalize(midpoint=0, vmin=density_min, vmax=density_max), cmap='bwr')
