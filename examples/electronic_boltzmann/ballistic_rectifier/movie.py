...
for file_number, dump_file in enumerate(moment_files[:]):
    ...
    pl.subplot(212)
    plot_grid(x[::1, :int(N_q2/2)], y[::1, :int(N_q2/2)], alpha=0.5)
    pl.contourf(x[:, :int(N_q2/2)], y[:, :int(N_q2/2)], (density[:int(N_q2/2), :]).T, 100,
        norm=MidpointNormalize(midpoint=0, vmin=density_min, vmax=density_max), cmap='bwr')
    #pl.title(r'Time = ' + "%.2f"%(time_array[file_number]) + " ps")
    #pl.colorbar()
    
    pl.xlim([-1, 1])
    pl.ylim([q2[0], q2[int(N_q2/2)-1]])
