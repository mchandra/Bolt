    ...
    if (q1_start_local_left != None and q2_start_local_bottom != None):
        ...
        # Bottom-left domain
            ...
            for i in range(0, N_q1 + 2*N_g):
                ...
                # Compute the x, y points using which the transformation will be defined
                # x, y nodes remain the same for each point on a vertical slice
                x_n           = x_0 + (2-radius/np.sqrt(2))*index/N # Top-left
                y_n           = -radius/np.sqrt(2)
                
    
                x_n_plus_1    = x_0 + (2-radius/np.sqrt(2))*(index+1)/N # Top-right
                y_n_plus_1    = -radius/np.sqrt(2)
    
                x_n_plus_half = x_0 + (2-radius/np.sqrt(2))*(index+0.5)/N # Top-center
        ...
        # Bottom-right domain
            ...
            for i in range(0, N_q1 + 2*N_g):
                ...
                # Compute the x, y points using which the transformation will be defined
                # x, y nodes remain the same for each point on a vertical slice
                x_n           = x_0 + (2-x_0)*index/N # Top-left
                y_n           = -radius/np.sqrt(2)
                
    
                x_n_plus_1    = x_0 + (2-x_0)*(index+1)/N # Top-right
                y_n_plus_1    = -radius/np.sqrt(2)
    
                x_n_plus_half = x_0 + (2-x_0)*(index+0.5)/N # Top-center
                y_n_plus_half = -radius/np.sqrt(2)
        ...
        # Top-left domain
            ...
            for i in range(0, N_q1 + 2*N_g):
                ...
                # Compute the x, y points using which the transformation will be defined
                # x, y nodes remain the same for each point on a vertical slice
                x_n           = x_0 + (2-radius/np.sqrt(2))*index/N # Bottom-left
                y_n           = radius/np.sqrt(2)
                
    
                x_n_plus_1    = x_0 + (2-radius/np.sqrt(2))*(index+1)/N # Bottom-right
                y_n_plus_1    = radius/np.sqrt(2)
    
                x_n_plus_half = x_0 + (2-radius/np.sqrt(2))*(index+0.5)/N # Bottom-center
                y_n_plus_half = radius/np.sqrt(2)
        ...
        # Top-right domain
            ...
            for i in range(0, N_q1 + 2*N_g):
                ...
                # Compute the x, y points using which the transformation will be defined
                # x, y nodes remain the same for each point on a vertical slice
                x_n           = x_0 + (2-x_0)*index/N # Bottom-left
                y_n           = radius/np.sqrt(2)
                
    
                x_n_plus_1    = x_0 + (2-x_0)*(index+1)/N # Bottom-right
                y_n_plus_1    = radius/np.sqrt(2)
    
                x_n_plus_half = x_0 + (2-x_0)*(index+0.5)/N # Bottom-center
                y_n_plus_half = radius/np.sqrt(2)
        ...
        # Center-Right domain
            ...
            for j in range(0, N_q2 + 2*N_g):
                ...
                x_y_bottom_left   = [x_n,         y_n]
                x_y_bottom_center = [(2 + x_n)/2, y_n]
                x_y_bottom_right  = [2,           y_n]
        
                x_y_left_center   = [x_n_plus_half, y_n_plus_half]
                x_y_right_center  = [2,             y_n_plus_half]
        
                x_y_top_left      = [x_n_plus_1,         y_n_plus_1]
                x_y_top_center    = [(2 + x_n_plus_1)/2, y_n_plus_1]
                x_y_top_right     = [2,                  y_n_plus_1]
        ...
        # Center-Left domain
            ...
            for j in range(0, N_q2 + 2*N_g):
                ...
                x_y_bottom_left   = [-2,           y_n]
                x_y_bottom_center = [(-2 + x_n)/2, y_n]
                x_y_bottom_right  = [x_n,          y_n]
        
                x_y_left_center   = [-2,            y_n_plus_half]
                x_y_right_center  = [x_n_plus_half, y_n_plus_half]
        
                x_y_top_left      = [-2,                  y_n_plus_1]
                x_y_top_center    = [(-2 + x_n_plus_1)/2, y_n_plus_1]
                x_y_top_right     = [x_n_plus_1,          y_n_plus_1]
